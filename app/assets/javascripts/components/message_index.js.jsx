var MessageInbox = React.createClass({
    ...
    render: function () {
        ...
        if (this.state.messages.messages.length === 0) {
            var Messageitem = <h1>받은 쪽지가 없습니다.</h1>;
        }
        else {
    ...
    render: function () {
        if (this.state.visible) {
            ...
            if (this.props.readOrNot) {
                return (
                    <li className="message-item">
                        <div className="message-title"
                             onClick={this.handleClick}>{this.props.message.substring(0, 70) + "..."}</div>
                    </li>
                );
            }
            else {
                return (
                    <li className="message-item">
                        <span onClick={this.handleNewClick}>{this.props.message.substring(0, 70) + "..."} <span
                            className="label label-danger">New</span> </span>
                    </li>
                );
            }
        }
